...
std::vector<std::future<void>> futures__;
...
void BlockMul(const Float* a, const Float* b, Float* c, const dim2& blockDimA,
    ...
    for (size_t row = 0; row != blockDimC.y; ++row) {
        ...
        for (size_t col = 0; col != blockDimC.x; ++col) {
            ...
            //~10% faster when multithreading enabled for single precision
            __m256 s = _mm256_setzero_ps();
            for (size_t x = 0; x < blockDimA.y; x += 8) {
                const auto boff = offsetB.x + col;
                __m256 bcol =
                    _mm256_set_ps(b[(x + offsetB.y) * dimB.y + boff],
                                  b[(x + 1 + offsetB.y) * dimB.y + boff],
                                  b[(x + 2 + offsetB.y) * dimB.y + boff],
                                  b[(x + 3 + offsetB.y) * dimB.y + boff],
                                  b[(x + 4 + offsetB.y) * dimB.y + boff],
                                  b[(x + 5 + offsetB.y) * dimB.y + boff],
                                  b[(x + 6 + offsetB.y) * dimB.y + boff],
                                  b[(x + 7 + offsetB.y) * dimB.y + boff]);
                __m256 arow =
                    _mm256_loadu_ps(&a[rowOffset * dimA.x + offsetA.x + x]);
                s = _mm256_fmadd_ps(arow, bcol, s);
            }
            float buf[8];
            _mm256_storeu_ps(buf, s);
             c[row * blockDimC.x + col] = buf[0] + buf[1] + buf[2] + 
...
#endif
    if(csv) {
        cout << size.y << ',' << size.x << ',' << blockSize 
             << ',' <<  precision << ',' << numThreads << ',' << end << endl;
    } else {
        // Print(size, c.data());
